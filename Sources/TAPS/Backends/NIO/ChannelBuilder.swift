...
@resultBuilder internal struct ProtocolStackBuilder<InboundOut, OutboundOut> {
    ...
    internal static func buildPartialBlock<PartialInboundOut, PartialOutboundIn>(
        ...
        ProtocolStack<_, _, _, _>(handlers: subprotocol.handlers)
    }
    
    internal static func buildPartialBlock<Handler: ChannelDuplexHandler>(
        first handler: Handler
    ) -> ProtocolStack<InboundOut, Handler.InboundOut, Handler.OutboundIn, OutboundOut> where InboundOut == Handler.InboundIn, OutboundOut == Handler.OutboundOut {
        ProtocolStack<_, _, _, _>(handlers: [ handler ])
    }
    ...
    {
        ProtocolStack<_, _, _, _>(handlers: base.handlers + [handler])
    }
